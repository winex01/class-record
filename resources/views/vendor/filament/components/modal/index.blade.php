...
<div
    ...
    x-data="{
        ...
        close: function () {
            const originalClose = filamentModal({ id: @js($id) }).close

            this.isOpen = false

            if (this.$refs.modalContainer) {
                this.$refs.modalContainer.dispatchEvent(
                    new CustomEvent('modal-closed', { id: @js($id) }),
                )
            }
            ...
            this.$nextTick(() => {
                ...
                if (openModals.length === 0) {
                    // Force clear all body scroll locks for nested modals
                    if (window.clearAllBodyScrollLocks) {
